...
describe('GET /v2/admin/quiz/{quizid} (adminQuizInfo)', () => {
  ...
  describe('Error cases', () => {
    test('Invalid session', () => {
      ...
      const res = reqAdminQuizInfoV2(invalidSession, quizId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
      expect(res.statusCode).toStrictEqual(401);
    });

    test('Empty session', () => {
      const res = reqAdminQuizInfoV2('', quizId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
      expect(res.statusCode).toStrictEqual(401);
    });
    ...
    test('Invalid quizId', () => {
      ...
      const res = reqAdminQuizInfoV2(session, invalidQuizId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
    ...
    test('Quiz belongs to another user', () => {
      ...
      const res = reqAdminQuizInfoV2(session2, quizId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
      expect(res.statusCode).toStrictEqual(403);
    });
  });
  ...
  describe('Success cases', () => {
    test('Correctly returns info for one quiz with no questions', () => {
      const res = reqAdminQuizInfoV2(session, quizId);
    ...
    describe('Multiple quizzes from multiple users', () => {
      ...
      test.each([
        ...
        const res = reqAdminQuizInfoV2(sessionField(), quizIdField());
        expect(res.body).toStrictEqual({
          quizId: quizIdField(),
          name: 'valid name',
          timeCreated: expect.any(Number),
