...
describe('DELETE /v2/admin/quiz/{quizId}/question/{questionId} (adminQuizQuestionDeleteV2)', () => {
  let session: string;
  let quizId: number;
  ...
  describe('Error cases', () => {
    test('Invalid session token', () => {
      ...
      const res = reqAdminQuizQuestionDeleteV2(invalidSession, quizId, questionId);
    ...
    test('Non-existent quizId', () => {
      ...
      const res = reqAdminQuizQuestionDeleteV2(session, nonExistentQuizId, questionId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
    ...
    test('Non-existent questionId', () => {
      ...
      const res = reqAdminQuizQuestionDeleteV2(session, quizId, nonExistentQuestionId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
    ...
    test('Deleting question from another user\'s quiz', () => {
      ...
      const res = reqAdminQuizQuestionDeleteV2(secondSession, quizId, questionId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
      expect(res.statusCode).toBe(403);
    });

    test('Delete failed if a game is not at END state', () => {
      const resQuestion = reqAdminQuizQuestionCreateV2(quizId, session,
      ...
      const res = reqAdminQuizQuestionDeleteV2(session, quizId, questionId);
      expect(res.body).toStrictEqual({ error: expect.any(String) });
  ...
  describe('Success cases', () => {
    test('Successfully deletes a question', () => {
      ...
      const quizInfo = reqAdminQuizInfoV2(session, quizId);
    ...
    test('Successfully remove a question from a quiz with multiple question', () => {
      const resQuestion2 = reqAdminQuizQuestionCreateV2(quizId, session,
      ...
      const resDelete = reqAdminQuizQuestionDeleteV2(session, quizId, questionId);
